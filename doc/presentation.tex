...
\item \textbf{Admissible Heuristic:} $h(x) \leq d(x,y) + h(y) = h^*(n)$, i.e. does not overestimate the total cost
\item Algorithm terminates when found path has, actual cost lower than the estimated cost of any path through any open node
...
\item key in the Priority Queue is h value
\item \textbf{Admissible Heuristic}: h value never over estimates g value
\item Terminates when found path has, actual cost (g value) lower than the estimated cost (h value) of any path through any open node
...
\end{center}
\begin{itemize}
\item $h\left(S_{(n, p)}\right) = \displaystyle\min _{X=S_{1,p+1} \ \cdots \ S_{B,p+1}} \left(trans(S_{(n,p)}, X) + h(X)\right)$
\item Used \textit{C++ set} to implement priority queue to avoid duplicate keys
\end{itemize}
\end{frame}
...
\item \textbf{Test 4:} 6 VMs, 12 phases (random workload, backward induction)
\begin{center}
\begin{table}[h]
\begin{tabular}{|c|c|c|c|}
\hline 
...
\end{table}
\end{center}
\item \# (relaxed nodes) decreases by a factor of 150
\end{itemize}
\end{frame}
\restoreframe
